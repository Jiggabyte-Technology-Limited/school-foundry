...
console.log('--- Database Seeding Script (Primary School - Realistic Data) ---');
...
async function run() {
  try {
    ...
    let totalStudents = 0;
    let t1FullPaid = 0, t1PartialPaid = 0, t1Unpaid = 0;
    let t2FullPaid = 0, t2PartialPaid = 0, t2Unpaid = 0;
    let t3FullPaid = 0, t3PartialPaid = 0, t3Unpaid = 0;
    ...
    for (const gid of gradeIds) {
      for (let i = 0; i < 12; i++) {
        ...
        await enrollStudent(studentId, yearId, gid, demoUser.id);

        const gradeFeeStructures = feeStructureIds.flat().find(f => f.gradeId === gid);
        ...
        const t3Start = new Date(TERMS[2].start);
        const t3End = new Date(TERMS[2].end);

        await insertStudentFee(studentId, gradeFeeStructures.feeStructureId, randomDate(t1Start, t1End), FEE_AMOUNT);

        const rand = Math.random();
        if (rand < 0.7) {
          const p1Id = await insertPayment(studentId, yearId, termIds[0], gid, 50000, randomDate(t1Start, t1End), demoUser.id);
          await insertReceipt(p1Id, 50000, 50000, 0, demoUser.id);
          t1FullPaid++;
        } else if (rand < 0.9) {
          const partialAmount = 25000 + Math.floor(Math.random() * 2) * 15000;
          const p1Id = await insertPayment(studentId, yearId, termIds[0], gid, partialAmount, randomDate(t1Start, t1End), demoUser.id);
        ...
        await insertStudentFee(studentId, gradeFeeStructures.feeStructureId, randomDate(t2Start, t2End), FEE_AMOUNT);

        const rand2 = Math.random();
        if (rand2 < 0.5) {
          const p2Id = await insertPayment(studentId, yearId, termIds[1], gid, 50000, randomDate(t2Start, t2End), demoUser.id);
          await insertReceipt(p2Id, 50000, 50000, 0, demoUser.id);
          t2FullPaid++;
        } else if (rand2 < 0.8) {
          const partialAmount = 15000 + Math.floor(Math.random() * 3) * 10000;
          const p2Id = await insertPayment(studentId, yearId, termIds[1], gid, partialAmount, randomDate(t2Start, t2End), demoUser.id);
        ...
        await insertStudentFee(studentId, gradeFeeStructures.feeStructureId, randomDate(t3Start, t3End), FEE_AMOUNT);

        const rand3 = Math.random();
        if (rand3 < 0.6) {
          const p3Id = await insertPayment(studentId, yearId, termIds[2], gid, 50000, randomDate(t3Start, t3End), demoUser.id);
          await insertReceipt(p3Id, 50000, 50000, 0, demoUser.id);
          t3FullPaid++;
        } else if (rand3 < 0.85) {
          const partialAmount = 20000 + Math.floor(Math.random() * 2) * 15000;
          const p3Id = await insertPayment(studentId, yearId, termIds[2], gid, partialAmount, randomDate(t3Start, t3End), demoUser.id);
          await insertReceipt(p3Id, 50000, partialAmount, 50000 - partialAmount, demoUser.id);
          t3PartialPaid++;
        } else {
          t3Unpaid++;
        }
    ...
    console.log(`- Term 1: ${t1FullPaid} full, ${t1PartialPaid} partial, ${t1Unpaid} unpaid`);
    console.log(`- Term 2: ${t2FullPaid} full, ${t2PartialPaid} partial, ${t2Unpaid} unpaid`);
    console.log(`- Term 3: ${t3FullPaid} full, ${t3PartialPaid} partial, ${t3Unpaid} unpaid`);
...
function cleanup() {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      db.run('DELETE FROM sqlite_sequence WHERE name IN ("payment_receipts", "payments", "students", "academic_years", "terms", "student_year_enrollment", "fee_structure", "student_fees")', (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
...
function getOrCreateDemoUser() {
  return new Promise((resolve, reject) => {
    db.get("SELECT id, full_name, username FROM users WHERE username = 'demo' LIMIT 1", (err, row) => {
      if (err) return reject(err);
      if (row) {
        resolve(row);
      } else {
